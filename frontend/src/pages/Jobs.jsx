    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md border border-gray-200">
        <div className="p-4 border-b border-gray-100">
          <h3 className="text-lg font-semibold text-text-main">Choose a category</h3>
          <p className="text-sm text-text-secondary mt-1">
            Select where to save {pendingSaveJob?.title ? `“${pendingSaveJob.title}”` : 'this job'}.
          </p>
        </div>
        <div className="p-4 max-h-72 overflow-y-auto space-y-2">
          {!isCreatingCategory ? (
              ...
              {saveCategories.map((cat) => (
                ...
                >
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-text-secondary">Save to this category</span>
                  </div>
                </label>
              ))}
              <button
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-sm font-semibold text-text-main mb-2">New Category Name</label>
                <input
                  type="text"
              ...
              {showSearchDropdown && (filteredJobs.length > 0 || filteredCompanies.length > 0) && (
                ...
                >
                  ...
                  {filteredJobs.length > 0 && (
                    <div>
                      <div className="sticky top-0 px-3 py-2 bg-gray-50 border-b border-gray-200">
                        <p className="text-xs font-semibold text-text-secondary uppercase tracking-wide">Jobs</p>
                      </div>
                  ...
                  {filteredCompanies.length > 0 && (
                    <div>
                      <div className="sticky top-0 px-3 py-2 bg-gray-50 border-b border-gray-200">
                        <p className="text-xs font-semibold text-text-secondary uppercase tracking-wide">Companies</p>
                      </div>
          ...
          <div ref={jobListRef} className="flex-1 lg:overflow-y-auto space-y-3 pr-2 custom-scrollbar pb-2">
            ...
            {filtered.map((job) => {
              ...
              return (
                <div
                  key={job.id}
                  ref={(el) => { if (el) jobCardRefs.current[job.id] = el }}
                  onClick={() => handleJobClick(job)}
                  className={`bg-card-white rounded-xl p-5 ${
                    isSelected ? 'border-2 border-primary' : 'border border-border-color hover:border-gray-300'
                  } shadow-soft cursor-pointer relative group transition-all ${
                    ...
                    <div className="flex items-start gap-3 flex-1">
                      ...
                      <div className="flex flex-col">
                        <h3 className={`text-lg font-bold ${
                          isSelected ? 'text-text-main' : 'text-text-main group-hover:text-primary'
                        } transition-colors`}>
                          {job.title}
                        </h3>
                        <p className={`text-sm font-medium ${
                          isSelected ? 'text-primary' : 'text-text-secondary group-hover:text-text-main'
                        }`}>
        ...
        <div className="hidden lg:flex lg:col-span-7 lg:h-full lg:min-h-0 lg:overflow-hidden">
          {loadingSelectedJob ? (
            <div className="bg-card-white rounded-xl border border-border-color shadow-soft h-full w-full flex items-center justify-center">
              <p className="text-text-secondary text-center">Loading job details...</p>
            </div>
          ) : selectedJob ? (
            <JobDetailContent 
              job={selectedJob} 
            ...
            <div className="bg-card-white rounded-xl border border-border-color shadow-soft h-full w-full flex items-center justify-center">
              <p className="text-text-secondary text-center">Click a job to view details</p>
            </div>
          )}
        </div>
      </div>
